...
def loeffler_DCT(x):
    ...
    temp[3], temp[4] = butterfly(temp[3], temp[4])
    #rotation stage
    temp[2], temp[3] = rotation(temp[3], temp[2], -np.pi/8)
    temp[3] *= -1
    temp[4], temp[7] = rotation(temp[7], temp[4], -np.pi/16)
    temp[5], temp[6] = rotation(temp[6], temp[5], -3*np.pi/16)
    #--------
    out[0], out[4] = butterfly(temp[0], temp[1])
    out[2] = scaleup(temp[2])
    out[6] = scaleup(temp[3])
    temp[4], temp[5] = butterfly(temp[4], temp[5])
    temp[6], temp[7] = butterfly(temp[6], temp[7])
    out[1] = scaleup(temp[4])
    out[3], out[5] = butterfly(temp[6], temp[5])
    out[7] = scaleup(temp[7])
    return out
